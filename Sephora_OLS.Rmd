...
Yesenia is a Sephora employee who spends most of her shifts matching foundation to customers' skin complexion. This is a time consuming process by hand. Yesenia
decides to build a statistical model to quickly predict the best foundation shade for any customer based on their complexion, undertone, and the oil content of their skin. Let's imagine she has compiled a **training dataset** of customers, their aformentioned features, and their self-reported perfect foundation match (this will be our response variable). Let's now simulate some data to represent this scenario:
...
- \( \hat{\beta}_0, \hat{\beta}_1, \dots, \hat{\beta}_n \): the estimated coefficients and intercept
...
The procedure for finding the coefficients \( \beta_0, \beta_1, \ldots \beta_n \) to projected our predictors onto a hyperplane is the same as for univariate linear regression, where they are projected onto a line.
This procedure is known as* **Ordinary Least Squares (OLS)**. 
...
As you can see, there are 5 columns of our design matrix, although we only have 3 predictors. 
Each row of our design matrix represents a customer, and each column represents his or her complexion, undertone, and skin type as well as an **intercept** term. 

The intercept (also called \( \beta_0 \)) is the baseline value of the response variable when all predictors are zero. 
To include the intercept in our linear model, we add a column of 1s to the design matrix. This allows the model to learn the intercept as a coefficient just like any other variable.
...
You may notice that **combination** skin is absent from the columns of the design matrix.
...
It turns out that there is a unique solution to finding the value of \( \boldsymbol{\hat\beta} \) that minimizes the norm of the residuals.
The shortest distance between the true values \( Y \) and the fitted values \( \hat{Y} \) occurs when we **orthogonally project** \( Y \) onto the column space of \( X \). In other words, we’re finding \( \hat{Y} \in \text{Col}(X) \) such that the residual vector \( Y - X\boldsymbol{\hat\beta}\) is orthogonal (perpendicular, $90^o$) to every column of \( X \). We find this by multiplying the residual vector by the transpose of the design and setting to 0:
\[
...
Yesenia sees from this visualization that foundation shade can be represented as a plane defined by customer's skin complexion, undertone, and skin type. The scalar value associated with foundation incorporates information from these all of these dimensions.
Lines from our data points to the plane represent residuals between the model fit and true foundation shade.
...
- **t value**  
  ...
  \]  
   \(H_0: \beta_j = 0\) represents the null hypothesis that the coefficient for parameter $\beta$ is 0. The a large T value rejects \(H_0: \beta_j = 0\), indicating the parameter has significant effect on the response.
...
- **F-statistic**  
  Tests whether the model explains more variance than a model with no predictors:
  \[
...
Yesenia notices that there are three hyperplanes, one for each level of `skin_type`. To understand why we see this, let's solve for one $\hat{y}$ on the hyperplane $Y$:
for each value of`skin_type`:
...
Which of these coefficients are **interaction terms**? Which are the **main effects** (non-interaction terms)? 
> answer

Print the first 5 rows of the design matrix for this model using the `model.matrix` function. Look at observation for customer 1. Can you write the linear equation for this observation in matrix notation using the values of $X$ you see in the design matrix?

```{r, answerbonus}
# your answer here
```
## Solving for Interaction Effects

Let's say Yesenia wants to determine whether undertone has a different effect on foundation shade for people with oily skin than for people with dry skin and combination skin. She can test this in R using:

```{r}
# Fit a linear model including interaction between skin_type and undertone
interaction_model <- lm(foundation_shade ~ skin_complexion + skin_type * undertone, data = interaction_df)
...
Under the hood, we are solving for the coefficients `skin_typeDry:undertone` and `skin_typeOily:undertone`, represented by vectors from the design matrix.


- The coefficient `skin_typeDry:undertone` is associated with the vector:  
  \[
  \mathbf{x}_{\text{Dry:undertone}} = [0, 0, 0, 0, 0, 1, 0]
  \]

- The coefficient `skin_typeOily:undertone` is associated with the vector:  
  \[
  \mathbf{x}_{\text{Oily:undertone}} = [0, 0, 0, 0, 0, 0, 1]
  \]

Using the design matrix to solve the system of equations, these coefficients correspond to the **difference in slopes** of undertone between each skin type and the reference (combination skin):

\[
\beta_{\text{Dry:undertone}} = \left( \frac{\partial\, \hat{Y}}{\partial\, \text{undertone}} \right)_{\text{Dry}} - \left( \frac{\partial\, \hat{Y}}{\partial\, \text{undertone}} \right)_{\text{Combo}}
\]

\[
\beta_{\text{Oily:undertone}} = \left( \frac{\partial\, \hat{Y}}{\partial\, \text{undertone}} \right)_{\text{Oily}} - \left( \frac{\partial\, \hat{Y}}{\partial\, \text{undertone}} \right)_{\text{Combo}}
\]

These coefficients tell us whether the effect of undertone on foundation shade is **stronger or weaker** for Dry or Oily skin compared to Combination skin.
...
*Challenge* What if we wanted to test if the interaction effect of skin type and undertone is statistically significant between oily and dry skin, instead of compared to the baseline? Look up the documentation for `linearHypothesis` from the **car** package. Write how you would test this hypothesis in R:
...
For more information on diagnostics plots, see [@uva_diagnostics]. For an explanation of leverage and Cook's Distance, see  [@stack_hatmatrix].
...
\[
\text{foundation_shade} = \beta_0 
+ (\beta_1 \cdot \text{skin_complexion}) 
+ (\beta_2 \cdot \text{undertone}) 
+ (\beta_3 \cdot \text{skin_typeDry})
+ (\beta_4 \cdot \text{skin_typeOily})
+ (\beta_5 \cdot \text{skin_complexion} \times \text{undertone}) 
\]
...
Print the design matrix for the continuous interaction model using the `model.matrix` function. Write out the linear equation for observations 1 and 2. Can you see why the hyperplane is curving?
